...
class WindowAttention_GMM(nn.Module):
    """ Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
    ...
    def forward(self, x, mask=None, Hp = None, Wp = None):
        ...
        # attn = torch.clamp(torch.abs(self.pi[:, :H_grid, :W_grid,:,:,:]), max = 1.)*torch.exp(attn)
        # attn = attn.view(-1, self.num_heads, N, N)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=attn_mask, Hp = Hp, Wp = Wp)  # nW*B, window_size*window_size, C
