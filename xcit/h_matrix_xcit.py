...
class H_Matrix_1D:
    def __init__(self,
        ...
        print(f'<H_Matrix_New mask_type[{mask_type}] n[{n}] levels[{levels}] cls_token_count[{cls_token_count}] cls_token_pos[{cls_token_pos}]>')
        
        pos1d = np.arange(n)
        dist1d = pos1d[None] - pos1d[:, None]
        
        
        cross_dig = Matrix.get_cross_matrix(t=t, levels=levels)
        # print('> crossq')
        # Matrix.plot_matrix(cross_dig, 1, [600, 640]).show()
...
# %%
mesh = np.stack(np.meshgrid(np.arange(n), np.arange(n))[::-1], -1)
mesh.shape

mesh[..., 1]

# %%
a = np.floor(mesh / (n / 2**1)).astype(int)
(a[..., 0] == a[..., 1]).astype(int)

# %%
a = np.floor(mesh / (n / 2**2)).astype(int)
(a[..., 0] == a[..., 1]).astype(int)

# %%
a = np.floor(mesh / (n / 2**3)).astype(int)
(a[..., 0] == a[..., 1]).astype(int)

# %%
def find_factor(v, f):
    assert isinstance(f, int) and f >= 2
    assert isinstance(v, int) and v >= 2
    p = np.sum(v % f ** (np.arange(int(np.floor(np.log(v) / np.log(f)))) + 1) == 0)
    p = int(p)
    return p

# n = 128 * 3
# n, t, int(2**t), n / int(2**t)
# find_factor(128, 2)

# %%
n = 16
_level = 2
# max_level = int(np.floor(np.log(n) / np.log(2)))
max_level = find_factor(n, 2)
assert _level <= max_level

a = np.floor(mesh[..., None] / n * 2**(np.arange(_level) + 1))
b = (a[..., 0, :] == a[..., 1, :]).astype(int)
b.shape
c = b.sum(-1)
Matrix.plot_matrix(c)


# %%
dist1d = np.abs(pos1d[None] - pos1d[:, None])
# n

Matrix.plot_matrix(dist1d, (600, 660))
