...
class Attention_FiAK_Base(nn.Module):
    ...
    @classmethod
    def get_dist_fn(cls, use_gpytorch=False):
        if use_gpytorch:
            from gpytorch.kernels.kernel import Distance
            D = Distance()
            def _fn(q, k):
                return D._sq_dist(q, k, postprocess = False)
            return _fn
        else:
            def _fn(q, k):
                return torch.square(torch.cdist(q, k))
            return _fn
...
class Attention_FiAK_KQ(Attention_FiAK_Base):
    def __init__(self,
        ...
        '''
        Q pruning for FiAK - more realistic?
        '''
...
# %%
class VisionTransformer_FiAK(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        if len(kwargs) > 0:
            print('DeiT FiAK found kwargs:', kwargs)
        ...
        # TODO: calc and feed internal parameters: seq_len
